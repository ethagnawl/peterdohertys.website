...
projects, but it had been a few years and those had all been relatively simple
REST APIs.

This project was a CMS which allowed admins to produce and publish static web
pages to the Google Cloud Platform. In contrast, the majority of my development
experience over the last few years (er decade, but who's counting?) has been
with Ruby/Rails and AWS/Heroku.

Below you'll find a pros and cons list which attempts to summarize my thoughts
and feelings about the Python/Django/GAE/etc. ecosystem. It's likely that I've
overlooked things or that I'm just wrong about some things, so I'd be very
happy to be corrected by anyone taking the time to read this (PR, tweet,
whatever you like). It's also worth noting that, for reasons which were out of
my control, I was using older versions of Python (2.x) and Django (1.x) on this
project, so some of the following criticisms may no longer be relevant.
...
GitHub and the Google mailing list), /r/djangolearning and Stack Overflow.
...
the libraries you are using. As a result, I found myself reflexively seeking
out framework/library source code -- opposed to leaning on documentation --
more than I'm used to while working on this project.

There are many possible reasons that the Ruby community doesn't do this, but I
wonder how much of it is rooted in Ruby's (historical and perceived) use of
metaprogramming? Early versions of Rails used lots of "magic" and it wouldn't
necessarily be helpful to reference source when discussing solutions to
application level concerns and it's possible that this has bled out into the
larger community, but that's just a hunch.

On the flip side, I've found that the Ruby community favors tutorials, working
...
able to write useful, exploratory, executable specs and found it easy to stub
out collaborating services (GAE Users, Cloud Datastore, etc.) The OS
integration was also very tight and Headless Chrome makes for a fast and,
generally, fantastic execution environment.

##### Flake8
The Flake8 linter is very helpful. It prevents superficial arguments about
coding style and can help you catch defects before you leave your text editor.
The [flake8 Vim plugin](https://github.com/nvie/vim-flake8) works well and
helped me discover (many) silly mistakes before leaving my editor.
...
community is very supportive and, for the most part, everything _Just Worked_.
...
be able to (CLI args and config file). This resulted in TONS of output (mostly
Djangae warnings, IIRC) when the tests were run and made it very difficult to
follow along with the test run or find debug information. Running individual
tests helped, but still resulted in multiple screens full of useless output.
...
ways to setup a Python development environment (install interpreters, install
dependencies, create workspaces, etc.). FWIW, we settled on virtualenv, pyenv
and pip on this project, but [pipenv](https://hackernoon.com/reaching-python-development-nirvana-bb5692adf30c?gi=fcb0c7412cb9) looks very promising for future projects.
...
which offers a similar feature set, but doesn't seem to be very widely used and
Django doesn't provide any Pake tasks for common operations.

##### Self
In my opinion, one of Python's biggest shortcomings -- especially when using
libraries/frameworks which require OOP -- is having to explicitly declare the
`self` parameter in method signatures. This is clunky and it's too easy to
forget to do this. Python 3 should have fixed this, while making other breaking
changes, by providing something similar to Ruby, JavaScript, Java, et al's
implicit self/this/that/whatever.

You get used to having to pass self to _every single instance method_ and yes,
you could use tooling to do it automatically, but why is that necessary? GVR
provides some justification for this approach in [this](https://neopythonic.blogspot.com/2008/10/why-explicit-self-has-to-stay.html) old blog post and
likely elsewhere, but the dynamic/metaprogramming edge cases could surely be
handled in other ways.

##### Tuples
Tuples are great, but tuple literals' required trailing commas are not. For
example, when passing optargs to a function or creating a tuple with one
...
or parenthesis used to denote precedence. This would be fine, if it were
the case for all datatype literals (list, dict, etc.), but this requirement
only applies to tuples and is a result of parentheses being overloaded. It
doesn't seem like the community is too aggravated by this, but it's another
syntax issue which could potentially have been addressed by Python 3.
...
into potential solutions, but there doesn't seem to be a way to tell the flake8
Vim plugin to ignore long lines.
...
event lifecycle can be also tapped into by third-party libraries to do useful
things, like generate tests, fixtures, documentation, etc. This prevents users
from having to copy code from the documentation to get started and prevents
users from having to copy code between files when creating new views, models,
etc.
...
actions, but it would be nice if Django provided a more flexible, robust
solution.

Also, as mentioned above, CBVs do sort of address this concern, but when you do
actually need concrete classes, it's nice to have them created in a uniform way.

##### Console
Django doesn't provide any `rails console` analogue. You can open a python
shell, manually include files and start poking around (I wound up writing a
shell script to do this) but this approach can be laborious and error prone.

UPDATE: It looks like Django does provide a similar utility (`python manage.py
shell`) which handles importing the expected project files and the like, but I
haven't used it, so YMMV.

##### Application File Structure
Compared to Rails, Django is _very_ unopinionated about application file
...
##### Asset Pipeline
Django doesn't ship with anything akin to Rails' Asset Pipeline for managing
static assets. The Asset Pipeline has been the subject of lots of debate, but
it does _Do The Right Thing_ in most cases and provides tons of utility for
free. There are libraries that you can use to have Django do stylesheet
preprocessing, asset fingerprinting, compression, etc. but you need to spend
time researching, integrating and supporting them. Ideally there would be a
default, gilded way to do these common operations.
...
lacking and, again, is hard to search for (see previous point).

For instance, I was never able to figure out how to make a GCS object publicly
visible _while_ uploading it using a signed URL. (The only alternatives were to
force the user to make a second request in order to get a new signed URL and
then use that URL in order to make the object public or just make the entire
bucket public, which is what we were, luckily, able to do.)

I was also unable to find a definitive answer to the question, "how long is the
timeout for HTTP requests running in background tasks?" It's possible that
these were just edge cases, but I saw comments on Stack Overflow and elsewhere
while searching for these answers that seemed to indicate that other developers
were used to seeing these sorts of omissions.
...
bucket contents, generates a response, etc. This seems like a glaring omission,
but we were unable to find a solution that didn't involve standing up a proxy.
Hopefully this is something Google will address in the future, because it's a
deal breaker for me -- S3 more/less does this by default.

#### Going Forward
Python is a perfectly pleasant and productive language to write programs in,
but I'm not sure it'd be my first choice for future projects of any
consequence. Python, Ruby and friends _all_ feel sloppy after using Elm for [a non-trivial
project](https://web.archive.org/web/20180928051336/https://www.intelfuturequiz.com/)
and [experimenting with Rust](https://github.com/ethagnawl/rust-c-ffi-demo),
though. ([GADTs](https://en.wikipedia.org/wiki/Generalized_algebraic_data_type) can't be unseen ...)

Simply put, interpreted/dynamic languages make it very difficult to
write correct, reliable and performant programs. Python 3.5 added support for
type hints, which is a step in the right direction and may help make
applications more robust, but I don't know if that feature has or will see much
adoption as a standard practice or in third-party libraries. The popular Python
3 implementations also still "feature" a global interpreter lock (GIL) and make
parallelism more complicated and expensive than it needs to be.

I'm hoping a sufficiently Rails/Django-like framework will emerge from within
either the Rust or Kotlin (may have already happened -- I haven't taken stock
recently) ecosystems. Otherwise, I'd take a good, hard look at a Java framework
which takes advantage of features introduced in 8+, a Haskell framework (Yesod,
Servant, Scotty, etc.) or Elixir/Phoenix.


#### Resources
* [https://www.agiliq.com/blog/2010/03/rails-and-django-commands-comparison-and-conversio/](https://www.agiliq.com/blog/2010/03/rails-and-django-commands-comparison-and-conversio/)
* [https://simpleisbetterthancomplex.com/](https://simpleisbetterthancomplex.com/)
* [https://spapas.github.io/2018/03/19/comprehensive-django-cbv-guide/](https://spapas.github.io/2018/03/19/comprehensive-django-cbv-guide/)
* [https://medium.com/django-musings/customizing-the-django-admin-site-b82c7d325510](https://medium.com/django-musings/customizing-the-django-admin-site-b82c7d325510)
* [https://www.techiediaries.com/customize-django-admin/](https://www.techiediaries.com/customize-django-admin/)
* [https://medium.com/better-programming/strings-unicode-and-bytes-in-python-3-everything-you-always-wanted-to-know-27dc02ff2686](https://medium.com/better-programming/strings-unicode-and-bytes-in-python-3-everything-you-always-wanted-to-know-27dc02ff2686)
